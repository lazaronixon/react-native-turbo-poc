(() => {
  ...
  class TurboNative {
    ...
    visitProposedToLocation(location, options) {
      if (
        window.Turbo &&
        typeof Turbo.navigator.locationWithActionIsSamePage === 'function'
      ) {
        if (
      ...
      this.postMessage('visitProposed', {
    ...
    visitRequestFailedWithStatusCode(visit, statusCode) {
      this.postMessage('visitRequestFailed', {
        identifier: visit.identifier,
    ...
    visitRendered(visit) {
      this.postMessageAfterNextRepaint('visitRendered', {
        identifier: visit.identifier,
      });
    }

    visitCompleted(visit) {
      this.postMessage('visitCompleted', {
        identifier: visit.identifier,
    ...
    formSubmissionStarted(formSubmission) {
      this.postMessage('formSubmissionStarted', {
        location: formSubmission.location.toString(),
      });
    }

    formSubmissionFinished(formSubmission) {
      this.postMessage('formSubmissionFinished', {
        location: formSubmission.location.toString(),
      });
    }
